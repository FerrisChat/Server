...
pub async fn edit_message(
    ...
    let resp = sqlx::query!("UPDATE messages SET content = $3, edited_at = now()::timestamp without time zone WHERE id = $1 AND id = $2 RETURNING *", bigint_message_id, bigint_channel_id, content)
        .fetch_optional(db)
        .await;
