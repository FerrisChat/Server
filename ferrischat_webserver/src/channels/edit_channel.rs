...
pub async fn edit_channel(req: HttpRequest, channel_info: Json<ChannelUpdateJson>, _: crate::Authorization) -> impl Responder {
    ...
    let resp = sqlx::query!("UPDATE channels SET name = $2 WHERE id = $1 RETURNING *", bigint_channel_id, name)
        .fetch_optional(db)
        .await;
